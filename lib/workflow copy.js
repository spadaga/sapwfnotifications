...
// CardFactory from botbuilder is no longer needed.
...
async function approveWorkflow(instanceId) {
 try {
   ...
   const response = await axios.post(
     ...
     {},
     { headers: { 'Authorization': `Bearer ${accessToken}`, 'Accept': 'application/xml,application/json', 'Content-Type': 'application/json' } }
   );
...
/**
 * Creates an Adaptive Card with a channel mention for better visibility.
 * @param {Object} workflow - The workflow object to display.
 * @param {Boolean} isLiveData - Whether this is live data or mock data.
 * @returns {Object} The raw Adaptive Card JSON object.
 */
function createIndividualApprovalCard(workflow, isLiveData = true) {
 try {
   ...
   // --- FIX: Conditionally map fields for mock OR live data ---
   let safeWorkflow;
   if (isMockData) {
     // Use the mock data field names
     safeWorkflow = {
   ...
   let companyCode = "N/A", amount = "N/A", documentType = "N/A";
   ...
   try {
     // The date format from SAP "26.05.2025" needs to be parsed correctly.
     ...
     if (parts.length === 3) {
        ...
        // Fallback for other date formats like ISO strings used in mock data
   ...
   cardBody.push({
     ...
     items: [
       { type: "ColumnSet", columns: [
         { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "Task Title:", weight: "bolder", size: "small", color: "dark" }] },
         { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.TaskTitle, size: "small", color: "dark", wrap: true }] }
       ], spacing: "small" },
       { type: "ColumnSet", columns: [
         { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "Status:", weight: "bolder", size: "small", color: "dark" }] },
         { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.Status, size: "small", color: "dark" }] }
       ], spacing: "small" },
       { type: "ColumnSet", columns: [
         { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "Instance ID:", weight: "bolder", size: "small", color: "dark" }] },
         { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: `${safeWorkflow.InstanceID}${isMockData ? " (MOCK)" : ""}`, size: "small", color: isMockData ? "attention" : "dark" }] }
       ], spacing: "small" },
       { type: "ColumnSet", columns: [
         { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "Created By:", weight: "bolder", size: "small", color: "dark" }] },
         { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.CreatedByName, size: "small", color: "dark" }] }
       ], spacing: "small" },
       { type: "ColumnSet", columns: [
         { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "Created On:", weight: "bolder", size: "small", color: "dark" }] },
         { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: formattedDate, size: "small", color: "dark" }] }
       ], spacing: "small" }
     ],
     style: "emphasis",
     spacing: "medium"
   ...
   const card = {
     ...
     body: cardBody,
     actions: [
       { type: "Action.Submit", title: isMockData ? "✓ Mock Approve" : "✓ Approve", data: { action: "approve", instanceId: safeWorkflow.InstanceID }, style: "positive" },
       { type: "Action.Submit", title: isMockData ? "✗ Mock Reject" : "✗ Reject", data: { action: "reject", instanceId: safeWorkflow.InstanceID }, style: "destructive" },
       { type: "Action.OpenUrl", title: isMockData ? "👁 Mock SAP Inbox" : "👁 View in SAP Inbox", url: safeWorkflow.InboxURL }
     ],
     msteams: {
      "entities": [
        {
          ...
          "mentioned": {
            "id": "channel",
            "name": "General" // The name here is just a placeholder; Teams uses the id 'channel'.
          }
        }
      ]
     },
